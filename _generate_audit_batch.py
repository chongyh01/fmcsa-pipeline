"""
_generate_audit_batch.py
========================
Generates 30 sample carrier reports for external audit (ChatGPT text review).

Stratified sampling — not pure random:
  Stratum A: Active for-hire, medium fleet (5-50 trucks)          — 8 carriers
  Stratum B: Active for-hire, owner-op (1-2 trucks)               — 5 carriers
  Stratum C: Inactive (revoked/lapsed authority likely)            — 7 carriers
  Stratum D: Active private/no-MC carriers                        — 4 carriers
  Stratum E: Large fleet (> 50 trucks)                             — 3 carriers
  Stratum F: Fleet asymmetry (one of trucks/drivers is 0)          — 3 carriers

Excludes the 8 gold_carriers.json DOTs.

Output: one .txt file per carrier + manifest.csv in:
  CODES/Carrier Report 1st July 2026/

Usage:
    python _generate_audit_batch.py
"""

import os, sys, csv, time, textwrap
import psycopg2
from pathlib import Path
from datetime import datetime
from dotenv import load_dotenv

load_dotenv(os.path.join(os.path.dirname(__file__), ".env"))
if sys.platform == "win32":
    sys.stdout.reconfigure(encoding="utf-8", errors="replace")

from carrier_facts import build_carrier_facts, NOT_FOUND
from validation_rules import run_validation_with_conflicts

DB_URL = os.getenv("SUPABASE_DB_URL", "")
if not DB_URL:
    print("ERROR: SUPABASE_DB_URL not set"); sys.exit(1)

OUTPUT_DIR = Path(__file__).parent / "Carrier Report 1st July 2026"
OUTPUT_DIR.mkdir(exist_ok=True)

GOLD_DOTS = {"204814", "3431540", "3612954", "3841767", "623336", "3012101", "1612145", "2308088"}

W = 80


# ── Stratified DOT sampling ──────────────────────────────────────────────────

def sample_dots(conn) -> list[dict]:
    """
    Pull candidate DOTs per stratum. Each stratum over-fetches (2-3x) so we
    can discard any that fail to build without going below target counts.
    Returns list of {dot, stratum} dicts, deduplicated.
    """
    gold_sql = "(" + ",".join(f"'{d}'" for d in GOLD_DOTS) + ")"
    strata = [
        ("A", f"""
            SELECT dot_number, 'A' AS stratum FROM carriers
            WHERE status = 'ACTIVE'
              AND mc_number IS NOT NULL AND mc_number NOT IN ('MC','')
              AND total_trucks BETWEEN 5 AND 50
              AND total_drivers > 0
              AND dot_number NOT IN {gold_sql}
            ORDER BY RANDOM() LIMIT 20
        """),
        ("B", f"""
            SELECT dot_number, 'B' AS stratum FROM carriers
            WHERE status = 'ACTIVE'
              AND mc_number IS NOT NULL AND mc_number NOT IN ('MC','')
              AND total_trucks BETWEEN 1 AND 2
              AND total_drivers BETWEEN 1 AND 2
              AND dot_number NOT IN {gold_sql}
            ORDER BY RANDOM() LIMIT 12
        """),
        ("C", f"""
            SELECT dot_number, 'C' AS stratum FROM carriers
            WHERE status = 'INACTIVE'
              AND mc_number IS NOT NULL AND mc_number NOT IN ('MC','')
              AND dot_number NOT IN {gold_sql}
            ORDER BY RANDOM() LIMIT 16
        """),
        ("D", f"""
            SELECT dot_number, 'D' AS stratum FROM carriers
            WHERE status = 'ACTIVE'
              AND (mc_number IS NULL OR mc_number IN ('MC',''))
              AND total_trucks > 0
              AND dot_number NOT IN {gold_sql}
            ORDER BY RANDOM() LIMIT 10
        """),
        ("E", f"""
            SELECT dot_number, 'E' AS stratum FROM carriers
            WHERE status = 'ACTIVE'
              AND mc_number IS NOT NULL AND mc_number NOT IN ('MC','')
              AND total_trucks > 50
              AND dot_number NOT IN {gold_sql}
            ORDER BY RANDOM() LIMIT 8
        """),
        ("F", f"""
            SELECT dot_number, 'F' AS stratum FROM carriers
            WHERE status = 'ACTIVE'
              AND (
                (total_trucks > 0 AND (total_drivers IS NULL OR total_drivers = 0))
                OR (total_drivers > 0 AND (total_trucks  IS NULL OR total_trucks  = 0))
              )
              AND dot_number NOT IN {gold_sql}
            ORDER BY RANDOM() LIMIT 8
        """),
    ]

    seen = set(GOLD_DOTS)
    rows = []
    cur = conn.cursor()
    for label, sql in strata:
        cur.execute(sql)
        for dot, stratum in cur.fetchall():
            if dot not in seen:
                seen.add(dot)
                rows.append({"dot": dot, "stratum": stratum})
    cur.close()
    return rows


STRATUM_TARGETS = {"A": 8, "B": 5, "C": 7, "D": 4, "E": 3, "F": 3}


def pick_30(candidates: list[dict]) -> list[dict]:
    """
    Greedily fill each stratum to its target.
    If a stratum runs short, make up the difference from stratum A or B leftovers.
    """
    by_stratum: dict[str, list] = {s: [] for s in STRATUM_TARGETS}
    for c in candidates:
        by_stratum[c["stratum"]].append(c)

    selected = []
    leftover = []
    for s, target in STRATUM_TARGETS.items():
        pool = by_stratum[s]
        chosen = pool[:target]
        selected.extend(chosen)
        leftover.extend(pool[target:])

    # Top up to 30 from leftovers if any stratum was short
    needed = 30 - len(selected)
    if needed > 0:
        selected.extend(leftover[:needed])

    return selected[:30]


# ── Report formatting ─────────────────────────────────────────────────────────

def fleet_bucket(pu: int) -> str:
    if pu == 0:   return "0 (unverified)"
    if pu <= 2:   return "1-2 (owner-op)"
    if pu <= 10:  return "3-10 (small)"
    if pu <= 50:  return "11-50 (medium)"
    if pu <= 200: return "51-200 (large)"
    return "200+ (very large)"


def _display_sentinel(val: str) -> str:
    """Bug 5: Replace bare NOT_FOUND with plain-language text."""
    if val == "NOT_FOUND":
        return "No record found in imported dataset — verify against SAFER before relying on this field"
    return val


def _display_authority_status(val: str) -> str:
    """Bug 5: NOT_FOUND for authority fields gets a more specific explanation."""
    if val == "NOT_FOUND":
        return ("No active authority record found in imported dataset. "
                "Possible causes: carrier never held authority, records not yet imported, "
                "or docket number mismatch. Verify against SAFER/L&I.")
    return val


def format_report(facts, results, conflicts, stratum, dot, ts) -> str:
    lines = []
    def p(s=""): lines.append(s)

    p("=" * W)
    p(f"CARRIER INTELLIGENCE REPORT — FMCSA DATA")
    p(f"Generated : {ts}")
    p(f"DOT       : {dot}  |  Stratum : {stratum}")
    p("=" * W)
    p()

    # ── Carrier identity ──────────────────────────────────────────────────────
    p("── CARRIER IDENTITY " + "─" * (W - 20))
    p(f"  Legal Name    : {facts.legal_name}")
    p(f"  DOT Number    : {facts.dot_number}")
    p(f"  MC Number     : {facts.mc_number or '(none)'}")
    p(f"  Carrier Type  : {facts.carrier_type}")
    p(f"  USDOT Status  : {_display_sentinel(facts.usdot_status)}")
    p()

    # ── Current authority ─────────────────────────────────────────────────────
    p("── CURRENT AUTHORITY " + "─" * (W - 21))
    p(f"  Required      : {facts.authority_required}")
    p(f"  Status        : {_display_authority_status(facts.authority_status)}")
    if facts.authority_status == "CONFIRMED_REVOKED":
        p(f"  Revoc Date    : {facts.authority_revocation_date or '—'}")
    p(f"  First Auth    : {facts.first_authority_date or '—'}")
    p(f"  Active Period : {facts.active_authority_period or '—'}")
    p()

    # ── Historical authority events ───────────────────────────────────────────
    # Bug 2: separate current status from historical events
    auth_events = [r for r in facts._authority_records if r.get("status")]
    alert_revocs = [a for a in facts._alerts
                    if "INVOLUNTARY_REVOCATION" in (a.get("event_type") or "").upper()]
    if auth_events or alert_revocs:
        p("── HISTORICAL AUTHORITY EVENTS " + "─" * (W - 30))
        for rec in auth_events:
            eff  = str(rec.get("effective_date") or "—")[:10]
            revd = str(rec.get("revocation_date") or "")[:10]
            stat = rec.get("status") or "—"
            rsn  = rec.get("reason") or ""
            line = f"  {eff}  {stat}"
            if revd:
                line += f"  (resolved {revd})"
            if rsn:
                line += f"  [{rsn}]"
            p(line)
        for alert in alert_revocs:
            edate = str(alert.get("event_date") or "—")[:10]
            desc  = alert.get("description") or "INVOLUNTARY REVOCATION"
            p(f"  {edate}  ALERT: {desc}")
        p()

    # ── Insurance ─────────────────────────────────────────────────────────────
    p("── INSURANCE " + "─" * (W - 13))
    p(f"  Required      : {facts.insurance_required}")
    p(f"  Status        : {_display_sentinel(facts.insurance_status)}")
    p(f"  Cancellation  : {facts.insurance_cancellation}")
    p(f"  Replacement   : {facts.insurance_replacement}")
    p()

    # ── Fleet & operations ────────────────────────────────────────────────────
    p("── FLEET & OPERATIONS " + "─" * (W - 22))
    p(f"  Power Units   : {facts.fleet_power_units}  ({fleet_bucket(facts.fleet_power_units)})")
    p(f"  Drivers       : {facts.fleet_drivers}")
    # Bug 3: explicitly label all counts as historical totals
    p(f"  Inspections   : {facts.inspection_count}  (historical total, all available records; deduped)")
    p(f"  Crashes       : {facts.crash_count}  (historical total, all available records)")
    p(f"  Violations    : {facts.violation_count}  (historical total, all available records)")
    # Bug 4: BOC-3 "NO" is not an authoritative negative
    if facts.boc3_on_file == "YES":
        p(f"  BOC-3 on File : YES")
    else:
        p(f"  BOC-3 on File : BOC-3 status not verified from imported data — manual L&I/MOTUS verification required")
    p(f"  SMS Data      : {'Present' if facts.sms_percentile_present else 'Not present in imported dataset'}")
    p()

    # ── Data confidence ───────────────────────────────────────────────────────
    p("── DATA CONFIDENCE " + "─" * (W - 19))
    for k, v in facts.data_confidence.items():
        p(f"  {k:<14}: {v}")
    p()

    # ── Validation results ────────────────────────────────────────────────────
    n_pass = sum(1 for r in results if r.passed)
    n_fail = sum(1 for r in results if not r.passed)
    p("── VALIDATION RULES " + "─" * (W - 20))
    p(f"  {n_pass}/{len(results)} rules passed | {n_fail} failed")
    p()
    for r in results:
        icon = "PASS" if r.passed else "FAIL"
        p(f"  [{icon}] Rule {r.rule_id:02d} {r.rule_name}")
        p(f"         {r.message}")
    p()

    # ── Conflict report ───────────────────────────────────────────────────────
    p("── VALIDATION CONFLICTS " + "─" * (W - 24))
    if not conflicts:
        p("  None detected.")
    else:
        p(f"  {len(conflicts)} conflict(s) found — manual SAFER verification recommended.")
        p()
        for i, c in enumerate(conflicts, 1):
            p(f"  [{i}] Rule : {c['rule']}")
            p(f"      Fields: {', '.join(c['fields'])}")
            wrapped = textwrap.fill(c["detail"], width=W - 12, initial_indent="      Detail: ", subsequent_indent="              ")
            p(wrapped)
    p()

    p("=" * W)
    p("Data source: FMCSA MCMIS via Socrata open data (data.transportation.gov)")
    p("Pipeline  : carrier_facts.py + validation_rules.py (Carrier Check USA)")
    p("=" * W)

    return "\n".join(lines)


# ── Main ──────────────────────────────────────────────────────────────────────

def main():
    conn = psycopg2.connect(DB_URL, connect_timeout=30)
    conn.autocommit = True

    print("Sampling DOTs...")
    candidates = sample_dots(conn)
    selected   = pick_30(candidates)
    print(f"  {len(selected)} DOTs selected across strata")

    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    manifest_rows = []
    written = []
    failed  = []

    for i, item in enumerate(selected, 1):
        dot     = item["dot"]
        stratum = item["stratum"]
        print(f"  [{i:02d}/30] DOT {dot} (stratum {stratum})", end=" ... ", flush=True)

        # Reconnect every 10 to avoid idle-timeout drops
        if i % 10 == 1 and i > 1:
            try: conn.close()
            except: pass
            conn = psycopg2.connect(DB_URL, connect_timeout=30)
            conn.autocommit = True

        try:
            facts = build_carrier_facts(conn, dot)
            if facts.legal_name == NOT_FOUND:
                print("NOT IN DB — skipped")
                failed.append(dot)
                continue

            results, conflicts = run_validation_with_conflicts(facts)
            n_fail = sum(1 for r in results if not r.passed)

            report_text = format_report(facts, results, conflicts, stratum, dot, ts)

            out_path = OUTPUT_DIR / f"DOT_{dot}.txt"
            out_path.write_text(report_text, encoding="utf-8")

            manifest_rows.append({
                "dot_number":         dot,
                "stratum":            stratum,
                "legal_name":         facts.legal_name,
                "carrier_type":       facts.carrier_type,
                "usdot_status":       facts.usdot_status,
                "authority_status":   facts.authority_status,
                "insurance_status":   facts.insurance_status,
                "fleet_power_units":  facts.fleet_power_units,
                "fleet_drivers":      facts.fleet_drivers,
                "fleet_bucket":       fleet_bucket(facts.fleet_power_units),
                "inspection_count":   facts.inspection_count,
                "sms_present":        facts.sms_percentile_present,
                "validation_failures": n_fail,
                "has_conflicts":      "YES" if conflicts else "NO",
                "conflict_rules":     "; ".join(c["rule"] for c in conflicts),
            })
            written.append(dot)
            status = f"OK  ({n_fail} fail{'s' if n_fail != 1 else ''}, {'CONFLICT' if conflicts else 'clean'})"
            print(status)

        except Exception as exc:
            print(f"ERROR — {exc}")
            failed.append(dot)

    conn.close()

    # ── Manifest ──────────────────────────────────────────────────────────────
    manifest_path = OUTPUT_DIR / "manifest.csv"
    if manifest_rows:
        fieldnames = list(manifest_rows[0].keys())
        with open(manifest_path, "w", newline="", encoding="utf-8") as f:
            w = csv.DictWriter(f, fieldnames=fieldnames)
            w.writeheader()
            w.writerows(manifest_rows)
        print(f"\nManifest written: {manifest_path.name}  ({len(manifest_rows)} rows)")

    # ── Summary ───────────────────────────────────────────────────────────────
    with_conflicts = [r for r in manifest_rows if r["has_conflicts"] == "YES"]
    print(f"\n{'=' * W}")
    print(f"DONE")
    print(f"  Reports written : {len(written)}")
    print(f"  Failed / skipped: {len(failed)}")
    print(f"  With conflicts  : {len(with_conflicts)}")
    print(f"  Output folder   : {OUTPUT_DIR}")
    print(f"{'=' * W}")

    print("\nSelected DOTs:")
    for r in manifest_rows:
        flag = " [CONFLICT]" if r["has_conflicts"] == "YES" else ""
        print(f"  DOT {r['dot_number']:<10} {r['stratum']}  {r['authority_status']:<22} fleet={r['fleet_power_units']:<4} {r['legal_name'][:35]}{flag}")

    if failed:
        print(f"\nFailed: {', '.join(failed)}")


if __name__ == "__main__":
    main()
