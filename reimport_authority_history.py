"""
Authority history re-import — truncate + clean reload from FMCSA Authority History (9mw4-x3tu)
=================================================================================================
Fixes duplicate rows from the original incomplete/resumed import (was 7.92M, should be ~5.05M).

- Truncates `authority_history` once at the start of a fresh run.
...
- Progress saved to fmcsa_cache/authority_history_v2_progress.json (separate
  from the stale cache left by the original importer).
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("reimport_authority_history.log", encoding="utf-8"),
...
PROGRESS_F = CACHE_DIR / "authority_history_v2_progress.json"
...
def to_rows(df):
    ...
    for r in df.to_dict(orient="records"):
        ...
        rows.append((
            dot,
            sv(r.get("mod_col_1") or r.get("authority_type")),
            sv(r.get("original_action_desc") or r.get("status")),
            safe_date(r.get("orig_served_date") or r.get("effective_date")),
            safe_date(r.get("disp_served_date") or r.get("revocation_date")),
            sv(r.get("disp_action_desc") or r.get("reason")),
        ))
    return rows
                ...
                with conn.cursor() as cur:
                    ...
                    psycopg2.extras.execute_values(
                        ...
                        """INSERT INTO authority_history
                           (dot_number, authority_type, status,
                            effective_date, revocation_date, reason)
                           VALUES %s""",
...
if PROGRESS_F.exists():
    ...
    completed = set()
    log.info("Fresh start — truncating authority_history table")
    conn = psycopg2.connect(DB_URL, connect_timeout=10)
    with conn:
        with conn.cursor() as cur:
...
for batch_start in range(0, len(pending), DL_BATCH):
    ...
    with ThreadPoolExecutor(max_workers=min(DL_THREADS, len(batch))) as pool:
        futs = {pool.submit(fetch_page, off): off for off in batch}
